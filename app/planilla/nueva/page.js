...
function NuevaPlanillaInner() {
  ...
  async function agregarArchivos(fileList) {
    ...
    if (!modoEdicion) {
      // La planilla todavía no existe: se sube recién al guardar.
      setArchivosNuevos((prev) => [...prev, ...nuevos]);
      return;
    }
    // En edición la planilla ya existe: subimos cada archivo al toque,
    // así el error (si lo hay) se ve al instante y no se pierde nada.
    for (const file of nuevos) {
      try {
        const fila = await subirAdjunto(planillaIdEdicion, file);
        setAdjuntosExistentes((prev) => [...prev, fila]);
      } catch (err) {
