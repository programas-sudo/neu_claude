...
export async function buscarNeumaticosAvanzado({ marca, medida, numero_serie, dot, recapado, conReparacion }) {
  let query = supabase
    ...
    .select("identificador, marca, medida, numero_serie, dot, modelo, instancia");
  ...
  let queryStock = supabase
    ...
    .select("marca, medida, numero_serie, dot, modelo, instancia")
...
export async function getRegistroStock({ marca, medida, numero_serie, dot, instancia }) {
...
// + el campo que corresponda (serie o dot) + instancia (para no
// mezclarlo con otra unidad física que comparta el mismo código).
export async function getHistorialNeumaticoExacto({ marca, medida, numero_serie, dot, instancia }) {
...
export async function getProveedorNeumatico({ marca, medida, numero_serie, dot, instancia }) {
...
// Orden para MOSTRAR el trayecto de un neumático (tabla en pantalla).
// A igual fecha, "sale" va ANTES que "entra"/"permanece": primero sale
// de un lado, después entra al otro, aunque para decidir cuál es la
// ubicación VIGENTE se use el criterio contrario (ver más abajo).
export function ordenarTrayectoParaMostrar(filasTrayecto) {
  const PRIORIDAD_DISPLAY = { sale: 0, permanece: 1, entra: 1 };
  return [...(filasTrayecto || [])].sort((a, b) => {
    if (a.fecha !== b.fecha) return a.fecha < b.fecha ? -1 : 1;
    return (PRIORIDAD_DISPLAY[a.accion] ?? 1) - (PRIORIDAD_DISPLAY[b.accion] ?? 1);
  });
}
...
export function getUbicacionVigente(filasTrayecto) {
  ...
  // Se reordena por fecha, y a IGUAL fecha, "entra" pesa más que
  // "permanece" y este más que "sale". Esto resuelve el caso de un
  // intercambio de neumáticos entre 2 vehículos el mismo día: sin este
  // criterio, cuál de las dos planillas se haya guardado último podía
  // definir mal el resultado (mostrando el neumático como "sin
  // asignar" cuando en realidad terminó instalado en el otro vehículo).
  const PRIORIDAD_ACCION = { entra: 2, permanece: 1, sale: 0 };
  const ordenadas = [...filasTrayecto].sort((a, b) => {
    if (a.fecha !== b.fecha) return a.fecha < b.fecha ? -1 : 1;
    return (PRIORIDAD_ACCION[a.accion] ?? -1) - (PRIORIDAD_ACCION[b.accion] ?? -1);
  });
  ...
  return {
    instalado: true,
    fecha: ordenadas[inicio].fecha,
    matricula: ultima.matricula,
    posicion: ultima.posicion,
  };
}

// Busca si ya existe(n) neumático(s) con esta misma marca + medida +
// serie/DOT (ignorando la instancia, para encontrar TODAS las unidades
// que comparten ese código) — para preguntar "¿es el mismo que ya
// tenés cargado, o uno distinto?" al entrar un dato que coincide.
export async function getInstanciasExistentes({ marca, medida, numero_serie, dot }) {
  if (!numero_serie && !dot) return [];
  ...
  const instanciasVistas = new Set((data || []).map((f) => f.instancia));

  // también hay que mirar el stock: un neumático puede existir ahí
  // sin haber estado nunca en un vehículo todavía.
  let queryStock = supabase.from("stock_neumaticos").select("*").is("fecha_salida", null);
  ...
  const resultado = [];
  for (const instancia of instanciasVistas) {
    const filasInstancia = data.filter((f) => f.instancia === instancia);
    const vigente = getUbicacionVigente(filasInstancia);
    const ultima = filasInstancia[filasInstancia.length - 1];
    resultado.push({
      instancia,
      marca: ultima.marca,
  ...
  for (const item of stock || []) {
    if (instanciasVistas.has(item.instancia)) continue; // ya está listado arriba
    resultado.push({
      instancia: item.instancia,
      marca: item.marca,
      modelo: item.modelo,
  ...
  return resultado.sort((a, b) => a.instancia - b.instancia);
}
...
/* =========================================================
   NORMALIZACIÓN: para que "295/80/22.5", "295/80R22.5" y
   "295-80-22.5" se reconozcan como la MISMA medida (se comparan
   por los números, ignorando separadores y letras). Para marca y
   modelo, solo se ignoran mayúsculas/minúsculas, espacios de más
   y acentos.
   ========================================================= */
...
export function normalizarMedida(txt) {
  if (!txt) return "";
  return txt.toString().replace(/[^0-9]/g, "");
}

// Valores ya usados de un campo (marca, medida o modelo), juntando
// planillas y stock, para sugerir mientras se escribe una fila nueva.
export async function getValoresHistoricos(campo) {
  const [{ data: dePlanillas, error: err1 }, { data: deStock, error: err2 }] = await Promise.all([
    supabase.from("planilla_neumaticos").select(campo).not(campo, "is", null),
    supabase.from("stock_neumaticos").select(campo).not(campo, "is", null),
  ]);
  if (err1) throw err1;
  if (err2) throw err2;

  const valores = new Set();
  for (const fila of [...(dePlanillas || []), ...(deStock || [])]) {
    const v = fila[campo];
...
/* =========================================================
   ...
   de marcas distintas — eso NO es un duplicado. La medida se
   compara normalizada (por números), así "295/80/22.5" y
   "295/80R22.5" cuentan como la misma medida.
   ========================================================= */
function claveIdentidad(fila) {
  const marca = normalizarTexto(fila.marca || "");
  const medida = normalizarMedida(fila.medida || "");
  const instancia = fila.instancia || 1;
  if (fila.numero_serie && fila.numero_serie.trim()) {
    return `${marca}|${medida}|S:${fila.numero_serie.trim().toUpperCase()}|${instancia}`;
  }
  if (fila.dot && fila.dot.trim()) {
    return `${marca}|${medida}|D:${fila.dot.trim().toUpperCase()}|${instancia}`;
  }
  return null;
}
...
// Recalcula TODO el historial de un vehículo, de la planilla más vieja a
// la más nueva, en orden cronológico real. Sirve como "red de seguridad"
// cuando se cargaron planillas viejas después de planillas más nuevas
// (el recálculo automático de una sola planilla siguiente puede no ser
// suficiente si se cargó un lote grande fuera de orden): esto reprocesa
// todo de punta a punta, así que el resultado no depende del orden en
// que se hayan tipeado las planillas, solo de sus fechas reales.
export async function recalcularHistorialCompleto(vehiculoId) {
  const { data: planillas, error } = await supabase
    .from("planillas")
