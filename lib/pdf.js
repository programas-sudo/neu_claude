...
// Las fuentes estándar de jsPDF (helvetica) no incluyen el símbolo "→"
// (ni otros símbolos fuera de Latin-1). Al usarlo, jsPDF calcula mal el
// ancho del texto: no lo ajusta al renglón y lo corta en el borde de la
// página, además de dibujar un carácter roto. Se reemplaza por "->",
// que sí es seguro en cualquier fuente.
...
function agregarTextoPaginado(doc, texto, x, y, maxWidth, lineHeight = 4.5) {
  const lineas = doc.splitTextToSize(limpiarTextoPdf(texto), maxWidth);
...
export function exportarEstadoActualPDF(vehiculo, estadoActual) {
  ...
  doc.setFontSize(10);
  doc.text(
    [`Matrícula: ${vehiculo.matricula}`, `Tipo de vehículo: ${vehiculo.tipo_vehiculo || "-"}`].map(
      limpiarTextoPdf
    ),
    14,
    24
  ...
  const filas = [...estadoActual]
    ...
    .map((f) =>
      [
        ...
        f.tiene_neumatico === false ? "" : f.recapado ? "Sí" : "No",
        f.tiene_neumatico === false ? "" : f.reparacion || "-",
        f.tiene_neumatico === false ? "" : f.proveedor_origen || "-",
      ].map(limpiarTextoPdf)
    );
  ...
  autoTable(doc, {
    startY: 34,
    head: [
      ["Posición", "Marca", "Modelo", "Medida", "Serie/DOT", "Estado", "% Desgaste", "Recapado", "Reparación", "Proveedor"],
    ],
    body: filas,
    styles: { fontSize: 8 },
  });
